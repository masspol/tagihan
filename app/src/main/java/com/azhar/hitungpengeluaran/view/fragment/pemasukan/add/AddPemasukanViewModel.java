...
public class AddPemasukanViewModel extends AndroidViewModel {
    ...
    public void addPemasukan(final String type, final String plg, final String note, final String date, final int price) {
        Completable.fromAction(new Action() {
                    @Override
                    public void run() throws Exception {
                        ModelDatabase pemasukan = new ModelDatabase();
                        pemasukan.tipe = type;
                        pemasukan.pelanggan = plg;
    ...
    public void updatePemasukan(final int uid, final String plg, final String note, final String date, final int price) {
        Completable.fromAction(new Action() {
                    @Override
                    public void run() throws Exception {
                        databaseDao.updateDataPemasukan(plg, note, date, price, uid);
                    }
                })
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
